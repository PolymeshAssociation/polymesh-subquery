...
describe("Tuples", () => {
  it("should work with rawTypes", () => {
    const Tuple = createClass(
      registry,
      "(HashMap<Text,i64>,(bool, Text), i64)"
    );
  ...
  it("should work with actual types", () => {
    ...
    expect(isTuple(tuple)).toBeTruthy();
    expect(
      extractTupleTypes(tuple, "(MyHashMap,(bool,Text),i64)")
    ).toStrictEqual(["MyHashMap", "(bool,Text)", "i64"]);
  });
...
describe("Map", () => {
  it("should work with rawTypes", () => {
    ...
    const BTreeMap = createClass(
      registry,
      "HashMap<(u32, bool),BTreeMap<i8,Text>>"
    );
...
describe("Enum", () => {
  it("should work with actual types", () => {
    const Enum = createClass(
      registry,
      JSON.stringify({ _enum: { Foo: "Text", Bar: "Vec<u32>" } })
    );
...
describe("Struct", () => {
  it("should work with actual types", () => {
    const Struct = createClass(
      registry,
      JSON.stringify({ foo: "Text", bar: "Vec<u32>" })
    );
  ...
  it("should work with actual types", () => {
    ...
    const Struct = createClass(
      registry,
      JSON.stringify({ foo: "MyHashMap", bar: "Vec<u32>" })
    );
    const struct: any = new Struct(registry);
    expect(isStruct(struct)).toBeTruthy();
    expect(
...
describe("serializeLikeHarvester", () => {
  ...
  it("should log found types", () => {
    ...
    expect(
      serializeLikeHarvester(
        new MyVec(registry, [{ foo: true, bar: 3 }]),
        "MyVec",
        logFoundType
      )
  ...
  it("should serialize dates like the harvester", () => {
    ...
    const notPrecise = new CompactMoment(
      registry,
      Date.parse("04 Dec 1995 00:12:00 GMT")
    );
    expect(
      serializeLikeHarvester(notPrecise, "Compact<Moment>", logFoundType)
    ).toBe("1995-12-04T00:12:00");

    const veryPrecise = new CompactMoment(
      registry,
      Date.parse("04 Dec 1995 00:12:00 GMT") + 1
    );
    expect(
      serializeLikeHarvester(veryPrecise, "Compact<Moment>", logFoundType)
    ).toBe("1995-12-04T00:12:00.001000");
  });
  ...
  it("should serialize AccountIds like the harvester", () => {
    ...
    expect(
      serializeLikeHarvester(account, "Compact<Moment>", logFoundType)
    ).toBe(
  ...
  it("should serialize empty tuples like the harvester", () => {
    ...
    expect(
      serializeLikeHarvester(new Empty(registry), "Empty", logFoundType)
    ).toBe(null);
  });
  ...
  it("should serialize Calls like the harvester", () => {
    ...
    expect(
      serializeLikeHarvester(extrinsic, "Call", logFoundType)
    ).toStrictEqual({
      call_args: [
        {
          name: "who",
  ...
  it("should serialize Vec<LookupSource> as a call argument like the harvester", () => {
    ...
    expect(
      ...
      "841226ea070c9577979ca2e854130fbe3253853c13c05943e09908312950275d",
    ]);
  });
  ...
  it("should serialize Balance like the harvester", () => {
    ...
    expect(
      serializeLikeHarvester(new Balance(registry, 20), "Balance", logFoundType)
    ).toBe(20);
  });
  ...
  it("should serialize Tuples like the harvester", () => {
    ...
    expect(
      serializeLikeHarvester(
        new MyTuple(registry, ["hello", 30, -40]),
        "MyTuple",
        logFoundType
      )
