...
WITH event_data AS (
  SELECT
    events.id,
    events.event_arg_1 as account_id,
    events.block_id as updated_block_id,
    events.event_id,
    events.block_id,
    events.created_at,
    events.updated_at,
    blocks.datetime as created_block_datetime
  FROM events
  JOIN blocks ON events.block_id = blocks.block_id::text
  WHERE event_id = 'SecondaryKeyLeftIdentity'
  ORDER BY created_at ASC
), account_data AS (
  SELECT
...
SELECT
  ...
  account_data.permissions_id,
  event_data.created_block_datetime,
...
WITH event_data AS (
  SELECT
    events.event_arg_1 as account_id,
    events.created_at as left_time
  FROM events
  WHERE events.event_id = 'SecondaryKeyLeftIdentity'
)
UPDATE accounts
SET
  identity_id = NULL
...
AND NOT EXISTS (
  SELECT 1
  FROM events
  JOIN LATERAL json_array_elements_text(events.event_arg_1::json) as relink_account_id ON TRUE
  WHERE events.event_id = 'SecondaryKeysAdded'
  AND events.created_at > (
    SELECT left_time 
    FROM event_data 
    WHERE event_data.account_id = relink_account_id
  )
  AND relink_account_id = accounts.id
);
