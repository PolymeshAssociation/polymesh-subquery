...
| D8 | ~~**Every timestamp column becomes `timestamptz`.**~~ **Revised 2026-09-10 → documentation only.** | 2026-09-01 / 2026-09-10 | **Original:** convert every `Date` column to `timestamptz` in `db/compat.sql` so GraphQL emits `+00:00`. **Revised:** SubQuery's only temporal scalar is `Date` → `timestamp without time zone`, no directive; a `compat.sql` `ALTER` is an unconditional breaking change for exact-string-equality consumers plus a generated artifact to keep in sync, while the stored instant is already correct. So instead: a parse-as-UTC docstring on `Block.datetime` (stating the rule for every `Date` field) and on the entitlement-critical fields (`tradeDate`, `valueDate`, the `expiry` fields, `filedAt`, `start`/`end`). Non-breaking, nothing converted. (A briefly-considered "convert only `Block.datetime` after D13 removes the domain copies" was dropped — ~13 named `Date` columns still survive D13, so a single conversion just reintroduces the inconsistency.) The `data_block_datetime_timestamp` expression index is separately replaced with a plain btree (A18, Phase 7.6) — a perf fix independent of the type. Epoch-integer companion question stays open (`architecture-review.md` §10.2). |
| D13 | **Domain entities record provenance as an `Event` relation, not a `Block` + index + `datetime` copy.** | 2026-09-10 | `createdEvent` / `updatedEvent` replace `createdBlock` / `updatedBlock` on ~60 entities; standalone `datetime` (17→1), `eventIdx` (21→3) and `extrinsicIdx` (5→1) come off. Genesis/seeded rows point at a synthetic seed `Event` rather than a discriminator column — which also fixes a live dangling-FK **[V]** (`genesisHandler` writes a `createdEventId` for an `Event` row that never existed). `Extrinsic.events` added. Folded into Phase 7, staged across four commits (7.3–7.6). ~215 type errors / ~37 files — the largest schema change in the programme. Resolved open decisions and the build sequence in [`implementation/13-entity-provenance.md`](./implementation/13-entity-provenance.md). |
