...
| D8 | ~~**Every timestamp column becomes `timestamptz`.**~~ **Revised 2026-09-10: documentation only — the columns stay `Date`.** | 2026-09-01 / 2026-09-10 | Times are UTC today but serialize without a zone marker, so a consumer parsing `"2021-11-05T13:56:36"` as local time shifts silently by its own offset. **Original decision:** convert the columns to `timestamptz` in `db/compat.sql` so GraphQL emits `+00:00`. **Revised:** SubQuery's only temporal scalar is `Date` and it generates `timestamp without time zone` with no directive to change it; the `compat.sql` `ALTER` is an unconditional breaking change for any consumer doing exact string comparison on a datetime and adds a generated artifact to keep in sync, while the stored instant is already correct — only the wire string lacks a marker. So instead: a schema docstring on `Block.datetime` (covering every `Date` field) and on the entitlement-critical fields (`tradeDate`, `valueDate`, the `expiry` fields, `filedAt`, distribution dates) telling consumers to parse as UTC (`new Date(value + 'Z')`). Non-breaking. The **companion question — whether to also expose an epoch integer — is deliberately left open**, with the trade-off written up in `architecture-review.md` §10.2. |
