...
| D8 | ~~**Every timestamp column becomes `timestamptz`.**~~ **Revised 2026-09-10 (twice) — see D13.** | 2026-09-01 / 2026-09-10 | **Original:** convert every `Date` column to `timestamptz` in `db/compat.sql` so GraphQL emits `+00:00`. **First revision:** documentation only — SubQuery's only temporal scalar is `Date` → `timestamp without time zone`, no directive; a schema-wide `compat.sql` `ALTER` is an unconditional breaking change for exact-string-equality consumers plus a generated artifact to keep in sync, while the stored instant is already correct. So: a parse-as-UTC docstring on `Block.datetime` and the entitlement-critical fields (`tradeDate`, `valueDate`, the `expiry` fields, `filedAt`, `start`/`end`). **Second revision:** once **D13** removes the `datetime` copy from the 16 domain entities, `Block.datetime` is the *only* timestamp left, so it **is** converted to `timestamptz` — one column, no schema inconsistency, and the block-range→id-range time-filter pattern needs a plain btree on it anyway. Both land in Phase 7 ([`implementation/13-entity-provenance.md`](./implementation/13-entity-provenance.md)). The epoch-integer companion question stays open (`architecture-review.md` §10.2). |
