 ...
 */
// Pulls the SubQuery ambient globals (`api`, `store`, `logger`) into this file's compilation,
// which `tsconfig.json` otherwise only provides to `src/**/*`
import '../../node_modules/@subql/types/dist/global';
import '../../node_modules/@subql/types-core/dist/global';
...
const recoverBatch = async (
  ...
  return {
    failed,
    lastScannedId: rows.length ? rows[rows.length - 1].id : lastId,
    recovered,
